import { AddLayerVersionPermissionInput } from "./AddLayerVersionPermissionInput";
import { AddPermissionInput } from "./AddPermissionInput";
import { CreateAliasInput } from "./CreateAliasInput";
import { CreateEventSourceMappingInput } from "./CreateEventSourceMappingInput";
import { CreateFunctionInput } from "./CreateFunctionInput";
import { DeleteAliasInput } from "./DeleteAliasInput";
import { DeleteEventSourceMappingInput } from "./DeleteEventSourceMappingInput";
import { DeleteFunctionInput } from "./DeleteFunctionInput";
import { DeleteFunctionConcurrencyInput } from "./DeleteFunctionConcurrencyInput";
import { DeleteLayerVersionInput } from "./DeleteLayerVersionInput";
import { GetAccountSettingsInput } from "./GetAccountSettingsInput";
import { GetAliasInput } from "./GetAliasInput";
import { GetEventSourceMappingInput } from "./GetEventSourceMappingInput";
import { GetFunctionInput } from "./GetFunctionInput";
import { GetFunctionConfigurationInput } from "./GetFunctionConfigurationInput";
import { GetLayerVersionInput } from "./GetLayerVersionInput";
import { GetLayerVersionByArnInput } from "./GetLayerVersionByArnInput";
import { GetLayerVersionPolicyInput } from "./GetLayerVersionPolicyInput";
import { GetPolicyInput } from "./GetPolicyInput";
import { InvokeInput } from "./InvokeInput";
import { InvokeAsyncInput } from "./InvokeAsyncInput";
import { ListAliasesInput } from "./ListAliasesInput";
import { ListEventSourceMappingsInput } from "./ListEventSourceMappingsInput";
import { ListFunctionsInput } from "./ListFunctionsInput";
import { ListLayerVersionsInput } from "./ListLayerVersionsInput";
import { ListLayersInput } from "./ListLayersInput";
import { ListTagsInput } from "./ListTagsInput";
import { ListVersionsByFunctionInput } from "./ListVersionsByFunctionInput";
import { PublishLayerVersionInput } from "./PublishLayerVersionInput";
import { PublishVersionInput } from "./PublishVersionInput";
import { PutFunctionConcurrencyInput } from "./PutFunctionConcurrencyInput";
import { RemoveLayerVersionPermissionInput } from "./RemoveLayerVersionPermissionInput";
import { RemovePermissionInput } from "./RemovePermissionInput";
import { TagResourceInput } from "./TagResourceInput";
import { UntagResourceInput } from "./UntagResourceInput";
import { UpdateAliasInput } from "./UpdateAliasInput";
import { UpdateEventSourceMappingInput } from "./UpdateEventSourceMappingInput";
import { UpdateFunctionCodeInput } from "./UpdateFunctionCodeInput";
import { UpdateFunctionConfigurationInput } from "./UpdateFunctionConfigurationInput";
export declare type InputTypesUnion = AddLayerVersionPermissionInput | AddPermissionInput | CreateAliasInput | CreateEventSourceMappingInput | CreateFunctionInput | DeleteAliasInput | DeleteEventSourceMappingInput | DeleteFunctionInput | DeleteFunctionConcurrencyInput | DeleteLayerVersionInput | GetAccountSettingsInput | GetAliasInput | GetEventSourceMappingInput | GetFunctionInput | GetFunctionConfigurationInput | GetLayerVersionInput | GetLayerVersionByArnInput | GetLayerVersionPolicyInput | GetPolicyInput | InvokeInput | InvokeAsyncInput | ListAliasesInput | ListEventSourceMappingsInput | ListFunctionsInput | ListLayerVersionsInput | ListLayersInput | ListTagsInput | ListVersionsByFunctionInput | PublishLayerVersionInput | PublishVersionInput | PutFunctionConcurrencyInput | RemoveLayerVersionPermissionInput | RemovePermissionInput | TagResourceInput | UntagResourceInput | UpdateAliasInput | UpdateEventSourceMappingInput | UpdateFunctionCodeInput | UpdateFunctionConfigurationInput;

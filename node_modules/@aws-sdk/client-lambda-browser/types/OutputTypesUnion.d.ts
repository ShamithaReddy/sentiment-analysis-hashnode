import { AddLayerVersionPermissionOutput } from "./AddLayerVersionPermissionOutput";
import { AddPermissionOutput } from "./AddPermissionOutput";
import { CreateAliasOutput } from "./CreateAliasOutput";
import { CreateEventSourceMappingOutput } from "./CreateEventSourceMappingOutput";
import { CreateFunctionOutput } from "./CreateFunctionOutput";
import { DeleteAliasOutput } from "./DeleteAliasOutput";
import { DeleteEventSourceMappingOutput } from "./DeleteEventSourceMappingOutput";
import { DeleteFunctionOutput } from "./DeleteFunctionOutput";
import { DeleteFunctionConcurrencyOutput } from "./DeleteFunctionConcurrencyOutput";
import { DeleteLayerVersionOutput } from "./DeleteLayerVersionOutput";
import { GetAccountSettingsOutput } from "./GetAccountSettingsOutput";
import { GetAliasOutput } from "./GetAliasOutput";
import { GetEventSourceMappingOutput } from "./GetEventSourceMappingOutput";
import { GetFunctionOutput } from "./GetFunctionOutput";
import { GetFunctionConfigurationOutput } from "./GetFunctionConfigurationOutput";
import { GetLayerVersionOutput } from "./GetLayerVersionOutput";
import { GetLayerVersionByArnOutput } from "./GetLayerVersionByArnOutput";
import { GetLayerVersionPolicyOutput } from "./GetLayerVersionPolicyOutput";
import { GetPolicyOutput } from "./GetPolicyOutput";
import { InvokeOutput } from "./InvokeOutput";
import { InvokeAsyncOutput } from "./InvokeAsyncOutput";
import { ListAliasesOutput } from "./ListAliasesOutput";
import { ListEventSourceMappingsOutput } from "./ListEventSourceMappingsOutput";
import { ListFunctionsOutput } from "./ListFunctionsOutput";
import { ListLayerVersionsOutput } from "./ListLayerVersionsOutput";
import { ListLayersOutput } from "./ListLayersOutput";
import { ListTagsOutput } from "./ListTagsOutput";
import { ListVersionsByFunctionOutput } from "./ListVersionsByFunctionOutput";
import { PublishLayerVersionOutput } from "./PublishLayerVersionOutput";
import { PublishVersionOutput } from "./PublishVersionOutput";
import { PutFunctionConcurrencyOutput } from "./PutFunctionConcurrencyOutput";
import { RemoveLayerVersionPermissionOutput } from "./RemoveLayerVersionPermissionOutput";
import { RemovePermissionOutput } from "./RemovePermissionOutput";
import { TagResourceOutput } from "./TagResourceOutput";
import { UntagResourceOutput } from "./UntagResourceOutput";
import { UpdateAliasOutput } from "./UpdateAliasOutput";
import { UpdateEventSourceMappingOutput } from "./UpdateEventSourceMappingOutput";
import { UpdateFunctionCodeOutput } from "./UpdateFunctionCodeOutput";
import { UpdateFunctionConfigurationOutput } from "./UpdateFunctionConfigurationOutput";
export declare type OutputTypesUnion = AddLayerVersionPermissionOutput | AddPermissionOutput | CreateAliasOutput | CreateEventSourceMappingOutput | CreateFunctionOutput | DeleteAliasOutput | DeleteEventSourceMappingOutput | DeleteFunctionOutput | DeleteFunctionConcurrencyOutput | DeleteLayerVersionOutput | GetAccountSettingsOutput | GetAliasOutput | GetEventSourceMappingOutput | GetFunctionOutput | GetFunctionConfigurationOutput | GetLayerVersionOutput | GetLayerVersionByArnOutput | GetLayerVersionPolicyOutput | GetPolicyOutput | InvokeOutput | InvokeAsyncOutput | ListAliasesOutput | ListEventSourceMappingsOutput | ListFunctionsOutput | ListLayerVersionsOutput | ListLayersOutput | ListTagsOutput | ListVersionsByFunctionOutput | PublishLayerVersionOutput | PublishVersionOutput | PutFunctionConcurrencyOutput | RemoveLayerVersionPermissionOutput | RemovePermissionOutput | TagResourceOutput | UntagResourceOutput | UpdateAliasOutput | UpdateEventSourceMappingOutput | UpdateFunctionCodeOutput | UpdateFunctionConfigurationOutput;
